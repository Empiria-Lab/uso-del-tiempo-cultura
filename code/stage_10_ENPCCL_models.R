...
df1 <- read.csv("data/tidy/enpccl_profiles_NSE.csv")
df2 <- read.csv("data/tidy/enpccl_profiles_grupos_etarios.csv")
df3 <- read.csv("data/tidy/enpccl_profiles_nivel_educ.csv")
df4 <- read.csv("data/tidy/enpccl_profiles_sexo.csv")
...
ols_1 <- lm(mean_times ~ profile_label + subsample, data = subset(merged_df, time_set == "teatro"))
ols_2 <- lm(mean_times ~ profile_label + subsample, data = subset(merged_df, time_set == "danza"))
ols_3 <- lm(mean_times ~ profile_label + subsample, data = subset(merged_df, time_set == "musica"))
ols_4 <- lm(mean_times ~ profile_label + subsample, data = subset(merged_df, time_set == "cine"))
ols_5 <- lm(mean_times ~ profile_label + subsample, data = subset(merged_df, time_set == "artesania"))
ols_6 <- lm(mean_times ~ profile_label + subsample, data = subset(merged_df, time_set == "arte"))
ols_7 <- lm(mean_times ~ profile_label + subsample, data = subset(merged_df, time_set == "ppoo"))
ols_8 <- lm(mean_times ~ profile_label + subsample, data = subset(merged_df, time_set == "religion"))

## Save models
pin_write(board_folder("results/models"), ols_1, name = "ENPCCL_ols_1", versioned = TRUE)
pin_write(board_folder("results/models"), ols_2, name = "ENPCCL_ols_2", versioned = TRUE)
pin_write(board_folder("results/models"), ols_3, name = "ENPCCL_ols_3", versioned = TRUE)
pin_write(board_folder("results/models"), ols_4, name = "ENPCCL_ols_4", versioned = TRUE)
pin_write(board_folder("results/models"), ols_5, name = "ENPCCL_ols_5", versioned = TRUE)
pin_write(board_folder("results/models"), ols_6, name = "ENPCCL_ols_6", versioned = TRUE)
pin_write(board_folder("results/models"), ols_7, name = "ENPCCL_ols_7", versioned = TRUE)
pin_write(board_folder("results/models"), ols_8, name = "ENPCCL_ols_8", versioned = TRUE)
...
bptest(ols_3) ## Robust SEs are NOT necessary
...
stargazer(
  ...
  header          = FALSE,
  style           = "ajps",
  out             = "results/tables/ENPCCL_OLS_models.tex",
  title           = "Efecto de los perfiles en el uso del tiempo en actividades culturales",
  model.names     = FALSE,
  ...
  model.numbers   = FALSE,
  omit.stat       = c("f", "ser"),
  column.labels   = c("Teatro", "Danza", "Música", "Cine", "Artesanía", "Arte", "PPOO", "Religión"),
  omit            = c("subsample"),
  add.lines = list(
    c("Errores robustos", "Sí", "Sí", "Sí", "Sí", "Sí", "Sí", "Sí", "Sí"),
    c("Sociodemográficos", "Sí", "Sí", "Sí", "Sí", "Sí", "Sí", "Sí", "Sí"),
    c("VIF",
